...
def train_dr(train_data, loss_func, init = 'random', seed = None):
...
def train_dr_multiple_aux(args):
    train_func, train_data, loss_func, seed, train_kwargs = args
    return train_func(train_data, loss_func, seed=seed, **train_kwargs)
def train_dr_multiple(train_func, train_data, loss_func, processes = None, reps = 10, pbar_file = None, seed = None, **train_kwargs):
    rng = np.random.default_rng(seed)
    ...
    with mp.Pool(processes=processes) as pool:
      args_lst = [(train_func, train_data, loss_func, seed, train_kwargs) for seed in seeds]
...
def compare_train_funcs(funcs_lst, args_lst, num_nodes = 4, num_patterns = 3, noise = 0.1, duplication = 20, iters = 50):
    ...
    for i in range(iters):
      ...
      trained_networks = [
          rates_to_network(trained_K_fret, trained_k_out, k_in) for trained_K_fret, trained_k_out, _, _ in training_results
      ]
...
#    init_method = 'random'
#
#    train_data = generate_training_data(stored_data, noise = 0.1, duplication=20)
#
#
#    print('Training data:', stored_data)
#
#
#    trained_K_fret, trained_k_out, res = train_dr_MC(train_data, rmse, init=init_method)
#    trained_K_fret2, trained_k_out2, res2 = train_dr(train_data, rmse, init=init_method)
#
#    # this output is probably too much for bigger networks
#    for i in range(2*num_nodes):
#        print(trained_k_out[i], np.round(trained_K_fret[i,:],3).tolist())
#    for i in range(2*num_nodes):
#        print(trained_k_out2[i], np.round(trained_K_fret2[i,:],3).tolist())
#    for d in map(np.array, it.product(*([[-1,1]]*num_nodes))):
#        pre = '*' if list(d) in map(list, stored_data) else ' '
#        out_dr, out_sr = calc_network_output_dr(d, trained_K_fret, trained_k_out)
#        print(pre, d.flatten(), out_dr.flatten(), out_sr)
#
#    # generate Network object with these connections
#    k_in = np.array([
#        kin for bit in stored_data[0] for kin in (max(bit, 0), -min(bit, 0))
...
#      pickle.dump(output, outfile)
    ...
    if train_MC:
      ...
      results_MC, train_seeds_MC = train_dr_multiple(train_dr_MC, train_data, rmse, processes = 3, seed = train_seed_base, pbar_file=pbar_file, reps=reps, **train_kwargs_MC)
    if train_GD:
      train_kwargs_GD = {}
      results_GD, train_seeds_GD = train_dr_multiple(train_dr, train_data, rmse, processes = 3, seed = train_seed_base, pbar_file=pbar_file, reps=reps, **train_kwargs_GD)
    ...
    with open(f'/scratch/jberlean/tmp/train{train_str}_{reps}x_seed={seed}.p','wb') as outfile:
      pickle.dump(output, outfile)
